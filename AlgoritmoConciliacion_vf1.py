...
def verificadorAplicacionBeneficio(df):
    if 'ESTADO_GIRO' in df.columns:
        filtro = df['ESTADO_GIRO'].isin(['Aprobado con giro', 'Renovado con giro'])
        df.loc[filtro, 'MeritoReconocido'] = (
            df.loc[filtro, 'NETAORD'].fillna(0) -
            df.loc[filtro, 'Pago ejecutado'].fillna(0) -
            df.loc[filtro, 'Reintegro ejecutado'].fillna(0))
        df.loc[filtro, 'NETAORDB'] = (
            df.loc[filtro, 'NETAORD'].fillna(0))
        df.loc[filtro, 'VerificadorBeneficio'] = (
            df.loc[filtro, 'NETAORD'].fillna(0) -
            df.loc[filtro, 'MeritoReconocido'] -
            df.loc[filtro, 'Pago ejecutado'].fillna(0) -
            df.loc[filtro, 'Reintegro ejecutado'].fillna(0)        )
        df.loc[filtro, 'VerificadorBeneficio'] = df.loc[filtro, 'VerificadorBeneficio'].apply(
            lambda x: 'Error beneficio' if x != 0 else x)
        filtro2 = df['ESTADO_GIRO'] == 'Aprobado valor cero'
        df.loc[filtro2, 'ESTADO CIVF'] = 'E'
    return df


# Condensador de aplicacion del beneficio
def condensar_estados_aplicacion(df):
  condicion_proceso_beneficio = [
      df['STT EJECUCION'] == 'PAGO A',
      df['STT EJECUCION'] == 'PAGO B',
      df['STT EJECUCION'] == 'PAGO B | REI B',
      df['STT EJECUCION'] == 'PAGO E',
      df['STT EJECUCION'] == 'PAGO M',
      df['STT EJECUCION'] == 'PAGO P | REI P',
      df['STT EJECUCION'] == 'PAGO PP | REI P',
      ...
      df['STT EJECUCION'] == 'PAGO ICX | REI ICX',
      df['STT EJECUCION'] == 'REI ICX']
  valores_proceso_beneficio = [
        'PAGO FSE',
        'PAGO FSE',
        'PAGO Y REINTEGRO PARCIAL FSE',
        'PAGO FSE',
        'PAGO FSE',
        'PAGO Y REINTEGRO PARCIAL FSE',
        'PAGO Y REINTEGRO PARCIAL FSE',
        'REINTEGRO FSE',
        'REINTEGRO FSE',
        'PAGO Y REINTEGRO PARCIAL ICETEX',
        'REINTEGRO ICETEX']
  df['STT FONDO'] = np.select(condicion_proceso_beneficio, valores_proceso_beneficio, default=np.nan)

  condicion_proceso_fondo = [
      df['STT FONDO'] == 'PAGO FSE',
      df['STT FONDO'] == 'PAGO Y REINTEGRO PARCIAL FSE',
      df['STT FONDO'] == 'REINTEGRO FSE',
      df['STT FONDO'] == 'PAGO Y REINTEGRO PARCIAL ICETEX',
      df['STT FONDO'] == 'REINTEGRO ICETEX']
  valores_proceso_fondo = [
        'FSE',
        'FSE',
        'FSE',
        'ICETEX',
        'ICETEX']
  df['STD FONDO'] = np.select(condicion_proceso_fondo, valores_proceso_fondo, default=np.nan)
  return df
...
# Generador de grafico de dispersion
def generar_grafico_dispersion(df, columna_x, columna_y, columna_hue=None, columna_style=None, titulo='', path_img=None):
    plt.figure(figsize=(10, 6))
    sns.scatterplot(
        data=df,
        x=columna_x,          # Eje X
        y=columna_y,          # Eje Y
        hue=columna_hue,      # Colores según la columna 'hue'
        style=columna_style,  # Diferente estilo según la columna 'style'
...
def generarReportePlantillaMatriculados(df):
    ...
    columnas_equiposTecnicos = [
        'RECIBO','Id  factura','NUM_DOCUMENTO','CODIGO_ESTUDIANTE','ESTADO CIVF','STD FONDO','STT FONDO','RESUL VAL2',
        'Nombre del Tercero','PROGRAMA','TELEFONO_CONTACTO','CELULAR','EMAIL_PERSONAL', 'EMAIL_INSTITUCIONAL',
        'FONDOICETEX','RESOLUCIONICETEX','VALORGIROICETEX','Valor Factura','Pago ejecutado','Reintegro ejecutado','MeritoReconocido']
    ...
    for columna in columnas_caracterizacion:
        if columna not in df.columns:
            print(f"La columna '{columna}' no existe en el DataFrame.")
            return None, None
    df_reporte1 = df[columnas_equiposTecnicos].copy()
    for columna in columnas_equiposTecnicos:
        if columna not in df.columns:
            print(f"La columna '{columna}' no existe en el DataFrame.")
            return None, None
    df_reporte2 = df[columnas_equiposTecnicos].copy()
    ...
    return df_reporte, df_reporte1, df_reporte2
...
df_final_20241 = verificadorAplicacionBeneficio(df_final_20241)
df_final_20241 = condensar_estados_aplicacion(df_final_20241)
df_final_20241['ESTADO CIVF'].fillna('Extemporaneos', inplace=True)
filtro_df_final_20241 = (
  df_final_20241
  .groupby(['ESTADO CIVF','STD FONDO'])
  .agg(Poblacion=('RECIBO', 'size'),
       Neta = ('MTRNETA', 'sum'),
       NetaOrdinaria = ('NETAORDB', 'sum'),
       ValorAplicado=('Pago ejecutado', 'sum'),
       ValorReintegro=('Reintegro ejecutado', 'sum'),
       ValorReconocido=('MeritoReconocido', 'sum'))
  .reset_index()
)
filtro_df_final_20241 = filtro_df_final_20241.rename(columns={
    'ESTADO CIVF': 'Estado de beneficio validado'
})
filtro_df_final_20241['Estado de beneficio validado'] = filtro_df_final_20241['Estado de beneficio validado'].replace(estado_beneficio)
...
img_path2 = "/content/grafico_dispersion.png"
...
with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
  ...
  filtro_df_final_20241.to_excel(writer, sheet_name='Generalidades', startrow=1, startcol=16, index=False)
  ...
  if not dfl_piam20241_sqci_final.empty:
    ...
    plantilla_matriculados, plantilla_caracterizacion, reporteTrabajoEquiposTecnicos = generarReportePlantillaMatriculados(df_piamfinalajustado)
    ...
    if reporteTrabajoEquiposTecnicos is not None:
      reporteTrabajoEquiposTecnicos.to_excel(writer, sheet_name='ReporteTrabajoEquiposTecnicos', index=False)
      print('Se ha generado el reporte de trabajo de los equipos técnicos')
